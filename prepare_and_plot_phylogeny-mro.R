...
df.1 = read.csv("mro-barcodes-2025-1.csv")
df.2 = read.csv("mro-barcodes-2025-2.csv")
tree = read.tree("mro-ncbi-tree-2025.nwk")
df.1[,2:ncol(df.1)] = 1 - df.1[,2:ncol(df.1)]
df.2[,2:ncol(df.2)] = 1 - df.2[,2:ncol(df.2)]
df = df.1
df[df.1 != df.2] = "?"

my.data = data.frame()
for(i in 1:nrow(df)) {
  my.data = rbind(my.data, data.frame(label=df[i,1], 
                                      state=paste0(df[i,2:ncol(df)], collapse="")))
}
...
# output to file
write.table(pairs[,c(4,5)], "mro-independent-transitions_all0.txt", quote=FALSE, row.names = FALSE)
...
plotHypercube.curated.unc.tree = function(tree.set, 
  ...
  this.plot = gheatmap(g.core, data.m, low="white", high="#000000",
                       colnames_angle=90, hjust=1, font.size=font.size) +
...
for(i in 1:length(my.tree$tip.label)) {
  new.df = rbind(new.df, matrix(tmp.df[[i]], ncol=9, nrow=1))
}
...
plotA = plotHypercube.curated.unc.tree(tree.plot) + 
  scale_fill_gradientn(colors = c("white", "red", "black")) +
  theme(plot.margin = unit(c(1, 1, 3, 1), "lines"))+ 
          coord_cartesian(clip = "off")

sf = 4
png("mro-phylogeny.png", width = 800*sf, height = 400*sf, res = 75*sf)
print(plotA)
dev.off()